...
const unsigned long AWAKE_TIME_MS = 30000;
...
void handleRoot() {
  ...
  html += "<title>ESP32 Camera Wakeup Test</title>";
  html += "</head>";
  html += "<body>";
  html += "<h1>ESP32 Camera Wakeup Test</h1>";
...
void setup() {
  ...
  Serial.println();
  Serial.println("Starting ESP32 Sleep Mode Camera Test...");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (initCamera()) {
      if (wasTriggeredByGPIO13()) {
        capturePhoto();
      } else {
        Serial.println("Power on or reset. Photo capture skipped.");
      }
    }
